...
template <typename T>
ScalarField<T>::ScalarField(int size_x, int size_y, int size_z, std::string id_string, std::string run_id, std::string savepath, int proc_num) 
...
template <typename T>
T ScalarField<T>::GetValue(int i, int j, int k) const
{
    return mArray[idx(i, j, k)];
}

template <typename T>
void ScalarField<T>::SetValue(T value, int i, int j, int k)
{
    mArray[idx(i, j, k)] = value;
}

template <typename T>
T ScalarField<T>::GetValueWrap(int i, int j, int k) const
{
...
template <typename T>
void ScalarField<T>::SetValueWrap(T value, int i, int j, int k)
{
    int ii = (i + mSizeX) % mSizeX;
    int jj = (j + mSizeY) % mSizeY;
    int kk = (k + mSizeZ) % mSizeZ;
...
template <typename T>
void ScalarField<T>::AddToValue(T additive_value, int i, int j, int k)
{
    mArray[idx(i, j, k)] += additive_value;
}

template <typename T>
void ScalarField<T>::SetToConstantValue(T value)
{
...
template <typename T>
void ScalarField<T>::SetLinearGradientZ(T bottom, T top)
{
    for (int k = 0; k < mSizeZ; ++k)
    {
...
template <typename T>
T ScalarField<T>::ComputeAverage() const
{
    double sum = 0.0;
    int total = 0;
    for (int k = 0; k < mSizeZ; ++k)
...
template <typename T>
void ScalarField<T>::WriteToTextFile(const int timestep) const
{
 ...
 */
 template<typename T>
std::string ScalarField<T>::construct_basepath(const int timestep) const
{
