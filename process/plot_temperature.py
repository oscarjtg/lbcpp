...
def collect_data(runId, timestep):
    ...
    for process in range(9):
        ...
        filename_pattern = f"./output/{runId:05d}_p{process:03d}_t{timestep:09d}_t.csv"
...
def plot_heatmap(data, runId, timestep):
    ...
    plt.savefig(f'process/temperature_run{runId}_timestep{timestep}.pdf')
    plt.savefig(f'process/temperature_run{runId}_timestep{timestep}.png', dpi=300)
    plt.show()
