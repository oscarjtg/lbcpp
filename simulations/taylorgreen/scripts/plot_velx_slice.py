...
# Calculate the average vertical X-Velocity profile
...
fig.colorbar(im, ax=ax1, label='X-Velocity')
ax1.set_xlabel('X-axis')
ax1.set_ylabel('Z-axis')
ax1.set_title(f"Heatmap for Run ID: {run_id}, Processor: {processor_number}, Timestep: {timestep}, Y-value: {y_value}")

# Plotting the average vertical X-Velocity profile
ax2.plot(average_profile, np.arange(nz))
ax2.set_xlabel('Average X-Velocity')
ax2.set_ylabel('Z-axis')
ax2.set_title('Average Vertical X-Velocity Profile')
